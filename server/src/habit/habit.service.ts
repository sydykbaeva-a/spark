...
export class HabitService {
    constructor(
        @InjectRepository(HabitEntity) private readonly habitRepo: Repository<HabitEntity>,
        @InjectRepository(HabitChildMapEntity) private readonly habitChildMapRepo: Repository<HabitChildMapEntity>
    ) { }

    // Habit methods
    async find() {
        return (await this.habitRepo.find()).sort((habit1, habit2) => habit1.habit_id - habit2.habit_id);
    }
    ...
    async saveHabitChildMap(habitChildMapEntity) {
        return await this.habitChildMapRepo.save(habitChildMapEntity);
    }

    // Find habits by user id (parent)
    async findHabitsByUser(userId: number): Promise<{ habit_name: string, child_id: number, child_name: string }[]> {
        ...
        if (!habitsByUser.length) {
            throw new NotFoundException(`No habits found for child with User ID: ${userId}`);
        }
